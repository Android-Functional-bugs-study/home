...
class Utils(object):
    ...
    def generate_html(self, path, html_path, run_count):
        line_list = []
        f_html = open(os.path.join(html_path, str(run_count) +
                      "_trace.html"), 'w', encoding='utf-8')
        f_style = open("style.html", 'r', encoding='utf-8')
        f_write_trace = open(os.path.join(
            path, "read_trace.txt"), 'r', encoding='utf-8')
        ...
        for img_file in img_list:
            if ".png" in img_file and self.devices[0].device_serial in img_file:
                ...
                device1_img = state_num+"_" + \
                    self.devices[1].device_serial+".png"
                action = self.find_action_in_file(state_num, lines_trace)
                if path == html_path:
                    line = "      <li><img src=\"screen/"+img_file+"\" class=\"img\"><p>" + \
                        action+"</p><img src=\"screen/"+device1_img+"\" class=\"img\"></li>"+'\n'
                else:
                    line = "      <li><img src=\""+run_count+"/screen/"+img_file+"\" class=\"img\"><p>" + \
                        action+"</p><img src=\""+run_count+"/screen/" + \
                        device1_img+"\" class=\"img\"></li>"+'\n'
