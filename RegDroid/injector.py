...
class Injector(object):
    ...
    def change_setting_before_run(self, event_count, strategy):
        ...
        if strategy == "network_immediate_1":
            ...
            event = Event(None, "network_immediate_1",
            ...
            event = Event(None, "location_lazy_1",
                          self.devices[1], event_count)
        elif strategy == "location_lazy_2":
            self.location_lazy_2()
            event = Event(None, "location_lazy_2",
                          self.devices[1], event_count)
            ...
            event = Event(None, "battery_immediate_1",
                          self.devices[1], event_count)
        elif strategy == "permssion_lazy_1":
            self.permssion_lazy_1()
            event = Event(None, "permssion_lazy_1",
                          self.devices[1], event_count)
        elif strategy == "developer_lazy_1":
            self.developer_lazy_1()
            event = Event(None, "developer_lazy_1",
                          self.devices[1], event_count)
    ...
    def inject_setting_during_run(self, event_count, strategy, request_flag):
        event = None
        if strategy == "network_immediate_1":
            setting_or_not = random.randint(
                0, self.setting_random_denominator/10)
            if setting_or_not == 0:
            ...
            if self.devices[1].wifi_state is True:
                setting_or_not = random.randint(
                    0, self.setting_random_denominator/10)
                if setting_or_not == 0:
                    print("network_lazy_1")
                    self.network_lazy_1()
                    event = Event(None, "network_lazy_1",
                                  self.devices[1], event_count)
        elif strategy == "network_lazy_2":
            if self.devices[1].wifi_state is True:
                setting_or_not = random.randint(
                ...
                if setting_or_not == 0:
                    ...
                    self.network_lazy_2()
                    event = Event(None, "network_lazy_2",
                                  self.devices[1], event_count)
        elif strategy == "sound_lazy_1":
            if self.devices[1].sound_state is True:
                setting_or_not = random.randint(
                    0, self.setting_random_denominator/10)
                if setting_or_not == 0:
                    print("sound_lazy_1")
                    self.sound_lazy_1()
                    event = Event(None, "sound_lazy_1",
                                  self.devices[1], event_count)
        elif strategy == "location_lazy_1":
            if self.devices[1].gps_state is True:
                setting_or_not = random.randint(
                    0, self.setting_random_denominator/10)
                if setting_or_not == 0:
                    print("")
            ...
            if setting_or_not == 0:
                ...
                event = Event(None, "display_immediate_1",
                              self.devices[1], event_count)
            ...
            if setting_or_not == 0:
                ...
                event = Event(None, "display_immediate_2",
                              self.devices[1], event_count)
        elif strategy == "permssion_lazy_1":
            if request_flag == 1:
                setting_or_not = 0
                ...
                setting_or_not = random.randint(
                    0, self.setting_random_denominator)
            if setting_or_not == 0 and self.devices[1].permission is True:
    ...
    def network_lazy_1(self):
        ...
        elif self.devices[1].wifi_state is False:
            device.use(description="Airplane mode").wait()
            device.use(description="Airplane mode").click()
            self.devices[1].wifi_state = True
        device.use.press("back")
    ...
    def network_lazy_2(self):
        ...
        if self.devices[1].wifi_state is True:
            device.use(
                description="Wi-Fi,Wifi signal full.,No internet.,AndroidWifi").wait()
            device.use(
                description="Wi-Fi,Wifi signal full.,No internet.,AndroidWifi").click()
    ...
    def location_lazy_1(self):
        ...
        device1(scrollable=True, instance=0).scroll.to(
            text="Security & Location")
        device1(text="Security & Location").click()
        device1(text="Location").wait(timeout=3.0)
        device1(text="Location").click()
        time.sleep(self.rest_interval*1)
        if self.devices[1].gps_state is True and device1(text="ON").count > 0:
            device1(text="ON").click()
            self.devices[1].gps_state = False
        elif self.devices[1].gps_state is False and device1(text="OFF").count > 0:
    ...
    def location_lazy_2(self):
        ...
        while device0(text="Apps & notifications").count < 0 or device1(text="Apps & notifications").count < 0:
            time.sleep(self.rest_interval*1)
        print("Security & Location")
        device0(scrollable=True, instance=0).scroll.to(
            text="Security & Location")
        ...
        if self.devices[1].gps_state is True:
            ...
            self.devices[1].gps_state = False
        elif self.devices[1].gps_state is False:
    ...
    def sound_lazy_1(self):
        ...
        elif self.devices[1].sound_state is False:
    ...
    def battery_immediate_1(self):
        ...
        device.use.open_quick_settings()
        self.devices[0].use.open_quick_settings()
        if self.devices[1].battery_state is False:
            ...
            device.use(className="android.widget.RelativeLayout",
                       instance=0).wait()
            device.use(className="android.widget.RelativeLayout",
                       instance=0).click()
            ...
            device.use(scrollable=True, instance=1).scroll.to(
        ...
        elif self.devices[1].battery_state is True:
            device.use(description="Battery Saver").wait()
            device.use(description="Battery Saver").click()
            device.use(description="Battery Saver").wait()
            device.use(description="Battery Saver").long_click()
    ...
    def battery_lazy_1(self):
        ...
        device.use.open_quick_settings()
        self.devices[0].use.open_quick_settings()
        if self.devices[1].battery_state is True:
            device.use(description="Battery Saver").wait()
            device.use(description="Battery Saver").click()
            self.devices[1].battery_state = False
        elif self.devices[1].battery_state is False:
            device.use(description="Battery Saver").wait()
            device.use(description="Battery Saver").click()
            self.devices[1].battery_state = True
        device.use.press("back")
    ...
    def display_immediate_2(self):
        self.devices[1].use(
            resourceId="com.android.systemui:id/recent_apps").long_click()
        time.sleep(self.rest_interval*1)
        self.devices[1].use(
            resourceId="com.android.systemui:id/recent_apps").long_click()
